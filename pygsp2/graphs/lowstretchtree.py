...
class LowStretchTree(Graph):
    ...
    def __init__(self, k=6, **kwargs):
        ...
        for p in range(1, k):
            ii = np.concatenate(
                (ii, ii + 4**p, ii + 2 * 4**p, ii + 3 * 4**p, [4**p - 1], [4**p - 1],
                 [4**p + (4**(p + 1) + 2) // 3 - 1], [
                     (5 * 4**p + 1) // 3 - 1
                 ], [4**p + (4**(p + 1) + 2) // 3 - 1], [3 * 4**p]))
            jj = np.concatenate((jj, jj + 4**p, jj + 2 * 4**p, jj + 3 * 4**p, [
                (5 * 4**p + 1) // 3 - 1
            ], [4**p + (4**(p + 1) + 2) // 3 - 1], [3 * 4**p], [4**p - 1], [4**p - 1],
                                 [4**p + (4**(p + 1) + 2) // 3 - 1]))
        ...
        coords = np.concatenate((XCoords[:, np.newaxis], YCoords[:, np.newaxis]),
                                axis=1)
        ...
        super(LowStretchTree, self).__init__(W, coords=coords, plotting=plotting,
                                             **kwargs)
