...
class Torus(Graph):
    ...
    def __init__(self, Nv=16, Mv=None, **kwargs):
        ...
        coords = np.concatenate((np.reshape(
            xtmp, (Mv * Nv, 1), order='F'), np.reshape(ytmp, (Mv * Nv, 1), order='F'),
                                 np.reshape(ztmp, (Mv * Nv, 1), order='F')), axis=1)
        ...
        super(Torus, self).__init__(W, coords=coords, plotting=plotting, **kwargs)
