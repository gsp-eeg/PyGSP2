...
def make_metro_graph(edgesfile='santiago_metro_stations_connections.txt', coordsfile='santiago_metro_stations_coords.geojson'):
    ...
    * Download the file `santiago_metro_stations_connections.txt` from this link:
    ...
    d = {x['properties']['name']: x['geometry']['coordinates'] for x in data['features']}
    ...
    for station, latlong in d.items():
        ...
        G.add_node(station, latlong=latlong, distance=distance(latlong, d[ref_station]).meters, x=x, y=y)
...
def metro_database_preprocessing(commutes, stations):
    ...
    metro_commutes.loc[metro_commutes['paradero'] == 'CAL Y CANTO', 'paradero'] = 'PUENTE CAL Y CANTO'
    metro_commutes.loc[metro_commutes['paradero'] == 'PARQUE OHIGGINS', 'paradero'] = "PARQUE O'HIGGINS"
    metro_commutes.loc[metro_commutes['paradero'] == 'PDTE PEDRO AGUIRRE CERDA', 'paradero'] = 'PRESIDENTE PEDRO AGUIRRE CERDA'
    metro_commutes.loc[metro_commutes['paradero'] == 'PLAZA MAIPU', 'paradero'] = 'PLAZA DE MAIPU'
    metro_commutes.loc[metro_commutes['paradero'] == 'RONDIZONNI', 'paradero'] = 'RONDIZZONI'
    metro_commutes.loc[metro_commutes['paradero'] == 'UNION LATINO AMERICANA', 'paradero'] = 'UNION LATINOAMERICANA'
...
def plot_signal_in_graph(G, signal, title='Graph Signal', label='', cmap='viridis', alpha=1):
    ...
    cbar.set_ticklabels([f'{label:.0f}' for label in [0, np.amax(signal) / 2, np.amax(signal)]])
    plt.title(title)
    plt.tight_layout()
    plt.set_cmap(cmap)
...
def get_with_retries(url, retries=3, backoff_factor=0.3, status_forcelist=(500, 502, 504), timeout=10):
    ...
    retry_strategy = Retry(total=retries, backoff_factor=backoff_factor, status_forcelist=status_forcelist,
