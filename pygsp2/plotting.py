...
def _plt_handle_figure(plot):
    ...
    def inner(obj, **kwargs):
        ...
        if kwargs['ax'] is None:
            ...
            if (hasattr(obj, 'coords') and obj.coords.ndim == 2
...
def _plt_plot_filter(filters, n, eigenvalues, sum, labels, ax, **kwargs):
    ...
    if eigenvalues:
        ...
        mpl, _, _ = _import_plt()
        ax.add_collection(
            mpl.collections.LineCollection(segs, transform=ax.get_xaxis_transform(),
                                           zorder=0, color=[0.9] * 3, linewidth=1,
                                           label='eigenvalues'))
...
def _plot_graph(G, vertex_color, vertex_size, highlight, edges, edge_color, edge_width,
    ...
    if backend == 'pyqtgraph':
        ...
        return _plt_plot_graph(G, vertex_color=vertex_color, vertex_size=vertex_size,
                               highlight=highlight, edges=edges, indices=indices,
                               colorbar=colorbar, edge_color=edge_color,
                               edge_width=edge_width, limits=limits, ax=ax, title=title,
                               cmap=cmap, alphan=alphan, alphav=alphav,
...
@_plt_handle_figure
def _plt_plot_graph(G, vertex_color, vertex_size, highlight, edges, edge_color,
                    edge_width, indices, colorbar, limits, ax, cmap, alphan, alphav,
                    edge_weights):
    ...
    if edges and (G.coords.ndim != 1):  # No edges for 1D plots.
        ...
        if edge_weights is not None:
            ...
            ax.add_collection(
                LineCollection(edges, linewidths=edge_width, colors=edge_color,
                               linestyles=G.plotting['edge_style'], zorder=1,
                               alpha=alphav))
    ...
    if G.coords.ndim == 1:
        ...
        sc = ax.scatter(*G.coords.T, c=vertex_color, s=vertex_size, marker='o',
                        linewidths=0, alpha=alphan, zorder=2, vmin=limits[0],
                        vmax=limits[1])
        if np.isscalar(vertex_size):
            size_hl = vertex_size
        else:
            size_hl = vertex_size[highlight]
        ax.scatter(*coords_hl.T, s=2 * size_hl, zorder=3, marker='o', c='None',
    ...
    if indices:
        for node in range(G.N):
            ax.text(
                *tuple(G.coords[node]),  # accommodate 2D and 3D
                s=node,
                color='white',
...
def _qtg_plot_signal(G, signal, edges, vertex_size, limits, title):
    ...
    if edges:
        ...
        if G.coords.shape[1] == 2:
            adj = _get_coords(G, edge_list=True)
            pen = tuple(np.array(G.plotting['edge_color']) * 255)
            g = qtg.GraphItem(pos=G.coords, adj=adj, symbolBrush=None, symbolPen=None,
                              pen=pen)
            view.addItem(g)
    ...
    if G.coords.shape[1] == 2:
        gp = qtg.ScatterPlotItem(G.coords[:, 0], G.coords[:, 1], size=vertex_size / 10,
...
def _plot_spectrogram(G, node_idx):
    ...
    spi = qtg.ScatterPlotItem(np.repeat(np.arange(G.N), M),
                              np.ravel(np.tile(np.arange(M), (1, G.N))), pxMode=False,
                              symbol='s', size=1, brush=cmap.map(spectr, 'qcolor'))
    v.addItem(spi)
