...
def compute_cheby_coeff(f, m=30, N=None, *args, **kwargs):
    ...
    for o in range(m + 1):
        c[o] = (2.0 / N *
                np.dot(f._kernels[i](a1 * num + a2), np.cos(np.pi * o *
                                                            (tmpN + 0.5) / N)))
...
def compute_jackson_cheby_coeff(filter_bounds, delta_lambda, m):
    ...
    for i in range(1, len(ch)):
        ch[i] = (2 / (np.pi * i)) * (np.sin(i * np.arccos(filter_bounds[0])) -
    ...
    for i in range(len(jch)):
        jch[i] = (1 / np.sin(alpha)) * ((1 - i /
                                         (m + 2)) * np.sin(alpha) * np.cos(i * alpha) +
                                        (1 /
                                         (m + 2)) * np.cos(alpha) * np.sin(i * alpha))
