...
the spatial domain. As a consequence, graph signals can be localized in both
...
by the eigenvectors of the Laplacian, :attr:`pygsp2.graphs.Graph.coherence`, is
...
for w, ax in zip([10, 1, 0.1, 0.01], axes.flatten()):
    ...
    ax.set_ylim(-1, 1)
    ax.set_yticks([-1, 0, 1])
    ax.legend([f'$u_{i}(v)$, $\lambda_{i}={graph.e[i]:.1f}$' for i in range(graph.n_vertices)], loc='upper right')
    ...
    for i in range(graph.n_vertices - 1):
        j = i + 1
        ax.text(i + 0.5, -1.15, f'$w_{{{i}{j}}} = {adjacency[i][j]}$', horizontalalignment='center')
